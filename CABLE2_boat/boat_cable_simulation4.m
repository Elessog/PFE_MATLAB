...
global  Wn1c Pn1c Wn1ca Wn1cb rode_number Nn1c Kdl Kpl...
    L vect_z boat_dot boat_dotdot index_out max_windspeed ...
    time_const_wind psi Lg mg delta_r delta_s controller_freq size_rect_cont;
...
if mod(t,1/controller_freq)<(1/controller_freq)*size_rect_cont
   [delta_r, delta_s] = controller_simpleLine_v_control(y_boat(1),y_boat(2), y_boat(4), y_boat(5),psi, a, b);
end

[dy_boat,alpha_cable] = model_sailboat_jaulin_modified4(y_boat,windspeed,psi,delta_s,delta_r,-force_cable);

boat_dot = [dy_boat(1:2);0];
boat_dotdot = [dy_boat(5)*cos(y_boat(4))+dy_boat(6);dy_boat(5)*sin(y_boat(4))+dy_boat(7);0];
