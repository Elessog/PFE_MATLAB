...
global Wn1c Pn1c Wn1ca Wn1cb rode_number Nn1c Kpl Kdl Kil lambdainverse...
    ...
    command_buffer_size delta_r delta_s...
...
%         %active otto_smith
%         active_os = 1;
%         pos_sum_t = y_s(4*3*rode_number+rode_number+1:4*3*rode_number+rode_number+2,i-1) -...
%             y_s(4*3*rode_number+rode_number+1:4*3*rode_number+rode_number+2,i-length_delay);
%         pos_sum = pos_sum_t';
%     end
    k_1 = F_xy(x(i),y(:,i));
...
%    pos_to_controller(i,:) = y(4*3*rode_number+rode_number+1:4*3*rode_number+rode_number+2,i)'+pos_sum;
end
f_cable=f_cable';
y= y';
