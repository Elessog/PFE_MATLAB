...
if ~database_charged
    ...
    cell_date =  char(zeros(length(valid_results),length('2012-12-21 21:12:05')));
    
    for i=1:length(valid_results)
        current_row = valid_results(i);
        cell_date(i,:) = valid_results(i).gps_time;
    ...
    [Y,M,D,H,MN,S] =datevec(datenum(cell_date,'yyyy-mm-dd HH:MM:SS')) ;
    time = (H-H_s)*3600+(MN-MN_s)*60+(S-S_s);
