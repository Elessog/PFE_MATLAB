...
omega_dot_v = zeros(length(1:10:length(x)-1),3);
v_down = zeros(length(1:10:length(x)-1),1);
v_dot_= zeros(length(1:10:length(x)-1),2);
f_rudder_= zeros(length(1:10:length(x)-1),2);
f_sail_= zeros(length(1:10:length(x)-1),2);
f_frict_=  zeros(length(1:10:length(x)-1),2);
...
for i=1:10:length(x)-1
    ...
    if print
        ...
        sing = sign(sin(theta_boat(i)-wind_save(i,2)));
        if sing==0
            sing = 1;
        end
        draw_boat([],s,pos_b(1),pos_b(2),theta_boat(i),sauv_delta(3,i),sing*abs(sauv_delta(4,i)));
...
%%
figure
plot(x(1:10:length(x)-1),[v_dot_ f_rudder_ f_sail_ f_frict_])
...
theta_c = mod(theta_boat(1:10:length(x)-1),2*pi);
figure
...
plot(x(1:10:length(x)-1),[v_dot_r f_rudder_r f_sail_r f_frict_r])
axis([time(1) 235 -2 2])
...
kmlStr = ge_track(t_xr*stepH/24/3600,pos_lat,pos_lont,zeros(1,length(x_r)),...
    ...
    'lineColor','#FF22FFFF',...
    'lineWidth',5,...
    'extendedData',{'Speed',v(1:10:length(x)-1);'Rudder_Act', sauv_delta(1,1:10:length(x)-1)});
